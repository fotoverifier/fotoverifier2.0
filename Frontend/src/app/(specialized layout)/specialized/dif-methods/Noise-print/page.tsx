...
const Specialized_NP= () => {
  ...
  { title: 'Image Forensics Techniques', url: 'https://example.com/image-forensics' },
  { title: 'Detecting Spliced Images', url: 'https://example.com/spliced-images' },
];
       ...
       {showExample ? (
        <div className={styles.tutorial_area}>
          ...
          <div className={styles.note}>
            Comment:
            A square in the middle of the image was cropped and saved with lower quality.
          </div>
        </div>
      ) : (
         ...
         <ul className={styles.reference_list}>
            {references.map((ref, index) => (
              <li key={index} className={styles.reference_item}>
                <span className={styles.reference_number}>{index + 1}.</span> {/* Adding the number */}
