...
import { FaUpload, FaHome, FaGithub, FaMoon, FaSun, FaGlobe } from "react-icons/fa";
...
export default function Sidebar_Alt() {
  ...
  return (
    <div className={`bg-white w-[7%] p-4 shadow-md border-r flex flex-col transition-all duration-300 relative`}>      
        ...
        {categories.map((category) => {
          ...
          return (
            <div key={category.slug} className="relative group cursor-pointer">
              <div
                onClick={() => category.slug.startsWith("http") ? window.open(category.slug, "_blank") : router.push(category.slug)}
              ...
              >
                <div className="flex justify-center items-center w-6 h-6">{category.icon}</div>
                <span
                  className="absolute left-full ml-2 px-2 py-1 bg-gray-800 text-white text-sm rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap"
                >
                  {category.name}
                </span>
              </div>
      ...
      <div className="mt-auto space-y-2">
        ...
        >
          {darkMode ? <FaSun size={20} className="text-yellow-500" /> : <FaMoon size={20} className="text-gray-600" />}
        </button>
